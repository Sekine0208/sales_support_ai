...
import type { AnalyticsKPI, Department, SalesUser } from '../../types/analytics';
...
interface KPIGridProps {
  ...
  groupBy: 'department' | 'user' | 'industry' | 'companySize';
  onSort?: (field: string, order: 'ascend' | 'descend') => void;
}

const KPIGrid: React.FC<KPIGridProps> = ({ data, groupBy, onSort }) => {
  ...
  const columns: ColumnsType<any> = [
    {
      title: groupBy === 'department' ? '部署' : 
             groupBy === 'user' ? '営業担当' :
             groupBy === 'industry' ? '業界' : '企業規模',
      dataIndex: 'name',
      key: 'name',
      fixed: 'left',
      width: 150,
      render: (text: string, record: any) => {
        if (groupBy === 'department' && record.department) {
          return record.department.name;
        } else if (groupBy === 'user' && record.user) {
          return (
            <Space>
    ...
    },
    {
      ...
      align: 'right',
      render: (value: number, record: any) => {
        const rate = getAchievementRate(value, record.kpi.negotiationCount * 0.35);
        return (
          <Space direction="vertical" size={0} style={{ width: '100%' }}>
            <span>{formatNumber(value)}</span>
    ...
    },
    {
      ...
      align: 'right',
      render: (value: number, record: any) => {
        const lostRate = record.kpi.negotiationCount > 0 
          ? Math.round((value / record.kpi.negotiationCount) * 100)
          : 0;
        return (
          <Space direction="vertical" size={0} style={{ width: '100%' }}>
            <span>{formatNumber(value)}</span>
            <Tag color={lostRate > 50 ? 'error' : 'warning'}>
              {lostRate}%
            </Tag>
          </Space>
        );
  ...
  return (
    <div className="kpi-grid">
      <Table
        ...
        summary={(pageData) => {
          const totalKPI = pageData.reduce((acc, curr) => ({
            callCount: acc.callCount + curr.kpi.callCount,
            connectCount: acc.connectCount + curr.kpi.connectCount,
            appointmentCount: acc.appointmentCount + curr.kpi.appointmentCount,
            visitCount: acc.visitCount + curr.kpi.visitCount,
            negotiationCount: acc.negotiationCount + curr.kpi.negotiationCount,
          ...
          totalKPI.negotiationUnitPrice = totalKPI.negotiationCount > 0
            ? Math.round(pageData.reduce((sum, curr) => 
                sum + curr.kpi.negotiationUnitPrice * curr.kpi.negotiationCount, 0
              ) / totalKPI.negotiationCount)
            : 0;

          totalKPI.orderUnitPrice = totalKPI.orderCount > 0
            ? Math.round(pageData.reduce((sum, curr) => 
                sum + curr.kpi.orderUnitPrice * curr.kpi.orderCount, 0
              ) / totalKPI.orderCount)
            : 0;
